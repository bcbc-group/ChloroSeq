...
for s in samples:
    ...
    try:
        with open(glob.iglob(os.path.join(opts.input,s,"*.gtf")).__next__()) as f:
...
for q, s in enumerate(samples):
    ...
    try:
        with open(glob.iglob(os.path.join(opts.input,s,"*.gtf")).__next__()) as f: #grabs first .gtf file it finds inside the sample subdirectory
